...
const Login = () => {
  ...
  const [hasRequesError, setHasRequesError] = useState<boolean>(false);
  ...
  const handleSubmit = async () => {
    ...
    if (login && password) {
      const response = await LoginService({ email: login, password: password });
      console.log("Res:", response);
      if (response) {
        setState(response.user);
        setHasRequesError(false);
      ...
      <ScrollView>
        ...
        <View>
          ...
          {hasRequesError && (
            ...
            >
              <Ionicons name="warning" color={Colors.support.warning} size={20} />
              <Text color={Colors.support.warning}>Verifique sua senha!</Text>
            </View>
          )}
          <Button
            as="default"
